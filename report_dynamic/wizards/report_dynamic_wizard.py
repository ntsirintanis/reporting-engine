...
class ReportDynamicWizard(models.TransientModel):
    _name = "mass.report.wizard"

    @api.model
    def fields_view_get(
        self, view_id=None, view_type="form", toolbar=False, submenu=False
    ):
        result = super(ReportDynamicWizard, self).fields_view_get(
            view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu
        )
        context = self._context
        if context.get("mass_report_object"):
            mass_obj = self.env["report.dynamic"]
            editing_data = mass_obj.browse(context.get("mass_report_object"))
            all_fields = {}
            xml_form = etree.Element("form", {"string": tools.ustr(editing_data.name)})
            xml_group = etree.SubElement(
                xml_form, "group", {"colspan": "6", "col": "6"}
            )
            ...
            for field in editing_data.field_ids:
                if field.ttype == "many2many":
                    ...
                    xml_group = etree.SubElement(
                        xml_group, "group", {"colspan": "6", "col": "6"}
                    )
                    etree.SubElement(
                        xml_group,
                        "separator",
                    ...
                    etree.SubElement(
                        xml_group,
                        "field",
                        {"name": "selection__" + field.name, "colspan": "4"},
                    )
                    etree.SubElement(
                        xml_group,
                        "field",
            ...
            etree.SubElement(
                xml_form, "separator", {"string": "", "colspan": "6", "col": "6"}
            )
    ...
    def create(self, vals):
        ...
        return super(ReportDynamicWizard, self).create({})
    ...
    def read(self, fields, load="_classic_read"):
        ...
        return super(ReportDynamicWizard, self).read(real_fields, load=load)
