...
class TestWizardReportDynamic(common.TransactionCase):
    ...
    def test_action_generate_reports_form(self):
        ...
        #lock
        wiz_lock = self.env['wizard.lock.report'].create({
            'report_id' : report.id,})
        ...
        with self.assertRaises(UserError) as e:
            BLI_template = self.env['report.dynamic'].create({
                ...
                'resource_ref':  self.env['base.language.install'].search(
                    [], limit=1).id,
        ...
        partner_startswith_d = self.env['res.partner'].create({
            'name' : 'Dperson'})
        self.RD_template.write({
            'resource_ref': "{},{}".format(res_partner_model.model, 
                partner_startswith_d.id)})
        self.RD_report.write({
            'resource_ref': "{},{}".format(res_partner_model.model, 
                partner_startswith_d.id)})
        content2 = self.section2._compute_dynamic_content()
